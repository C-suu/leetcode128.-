...
* `set`（集合）有个超强能力：**问“某个数在不在里面”的查找速度很快**
...
# 在这一题，为何：x-1 in s不进入循环？给出解释，并给出一个示例的表格！

这个问题问得**非常好，而且是这道题的核心思想** 👍
我会按 **“为什么 → 直观比喻 → 严谨解释 → 表格示例”** 四步来讲，保证能真正理解。

---

# 一、先直接回答的问题（一句话版）

> **当 `x-1 in s` 时不进入循环，是为了避免从“连续序列的中间”重复计算，保证每个连续序列只被统计一次，从而把时间复杂度控制在 O(n)。**
...
规则是：
👉 **只有站在“第一扇门”前的人，才允许从左数到右**。

* 站在 1 前面（前面没有 0）👉 ✅ 可以开始数
* 站在 2 前面（前面有 1）👉 ❌ 不允许
* 站在 3 前面（前面有 2）👉 ❌ 不允许
...
## 表格 1：判断 `x-1 in s` 的结果

|  x | x-1 | x-1 in s ? | 说明         | 进入 while 吗 |
| -: | --: | :--------: | ---------- | :--------: |
|  1 |   0 |      否     | 1 是连续序列的起点 |      ✅     |
|  2 |   1 |      是     | 2 在序列中间    |      ❌     |
|  3 |   2 |      是     | 3 在序列中间    |      ❌     |
...
## 表格 2：如果错误地让所有 x 都进入 while（会看到问题）

|   起点 x | while 数的过程 |        数了多少次 |
| -----: | ---------- | -----------: |
|      1 | 1→2→3→4    |            4 |
|      2 | 2→3→4      |            3 |
|      3 | 3→4        |            2 |
...
这已经不是 O(n) 了，而是 **接近 O(n²)**。

---

## 表格 3：使用 `x-1 not in s`（正确做法）

|   起点 x | 进入 while? | while 数的过程 |       数了多少次 |
| -----: | :-------: | ---------- | ----------: |
|      1 |     ✅     | 1→2→3→4    |           4 |
|      2 |     ❌     | —          |           0 |
|      3 |     ❌     | —          |           0 |
